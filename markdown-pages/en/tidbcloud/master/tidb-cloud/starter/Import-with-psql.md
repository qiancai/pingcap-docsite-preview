...
By default, the plain `pg_dump` data stream uses PostgreSQL `COPY` statements. The data is imported into PostgreSQL-compatible TiDB Cloud Starter through the PostgreSQL wire protocol.

PostgreSQL-compatible TiDB Cloud Starter supports `COPY` in text and CSV formats. Binary `COPY` is not supported.
...
- During the Limited Public Preview, PostgreSQL-compatible TiDB Cloud Starter supports migration using `psql` and `COPY`.
