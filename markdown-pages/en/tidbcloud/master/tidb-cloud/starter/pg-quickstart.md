...
>
> After access is enabled, you can create PostgreSQL-compatible TiDB Cloud Starter instances from the TiDB Cloud console.
...
To create a TiDB Cloud Starter instance, follow these steps:
...
3. To create a new PostgreSQL-compatible TiDB Cloud Starter instance, follow these steps:

    1. Click **Create Resource**.
    2. On the **Create Resource** page, select **Starter**.
    3. In the **Compatibility Mode** section, select **PostgreSQL Compatible**.
    4. Enter a name for the TiDB Cloud Starter instance, select the cloud provider and target region, and then click **Create**.

    > **Note:**
    >
    > PostgreSQL-compatible TiDB Cloud Starter instances are currently free during the limited public preview.
    >
    > The first 10 PostgreSQL-compatible TiDB Cloud Starter instances in your organization are eligible for the free quota. Each eligible instance includes the following monthly free quota:
    >
...
## Step 2: Connect to your TiDB Cloud Starter instance

PostgreSQL-compatible TiDB Cloud Starter instances support the PostgreSQL wire protocol, so you can connect to them using most PostgreSQL tools, drivers, and ORMs. The following steps use the PostgreSQL CLI client `psql` as an example.
...
The PostgreSQL CLI client `psql` opens and connects to your TiDB Cloud Starter instance.

## What's next

- To learn how to connect to your PostgreSQL-compatible TiDB Cloud Starter instance, see [Connect to a TiDB Cloud Starter or Essential instance](/tidb-cloud/connect-via-standard-connection-serverless.md).
