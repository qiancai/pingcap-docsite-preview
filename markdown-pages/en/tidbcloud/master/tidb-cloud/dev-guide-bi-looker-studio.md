...
This tutorial takes a TiDB Cloud Starter cluster as an example to show you how to connect to TiDB Cloud with Looker Studio.
...
**If you don't have a TiDB Cloud Starter cluster, you can create one as follows:**

- [Create a TiDB Cloud Starter cluster](/develop/dev-guide-build-cluster-in-cloud.md#step-1-create-a-tidb-cloud-cluster)

## Step 1. Import a dataset

You can import the S&P 500 dataset provided in the interactive tutorial of TiDB Cloud Starter.
...
3. Select your TiDB Cloud Starter cluster, and then click **Import Dataset** to import the S&P 500 dataset to your cluster.
...
1. On the [**Clusters**](https://tidbcloud.com/project/clusters) page, click the name of your TiDB Cloud Starter cluster to go to its overview page.
...
4. Download the [CA cert](https://letsencrypt.org/certs/isrgrootx1.pem).
    ...
    > **Tip:**
    >
    > TiDB Cloud Starter requires a secure TLS connection between the client and the cluster, so you need this CA cert for connection settings in Looker Studio.
...
3. In the **BASIC** setting pane, configure the connection parameters.
    ...
    - **Host Name or IP**: enter the `HOST` parameter from the TiDB Cloud Starter connection dialog.
    - **Port(Optional)**: enter the `PORT` parameter from the TiDB Cloud Starter connection dialog.
    - **Database**: enter the database you want to connect to. For this tutorial, enter `sp500insight`.
    - **Username**: enter the `USERNAME` parameter from the TiDB Cloud Starter connection dialog.
    - **Password**: enter the `PASSWORD` parameter from the TiDB Cloud Starter connection dialog.
