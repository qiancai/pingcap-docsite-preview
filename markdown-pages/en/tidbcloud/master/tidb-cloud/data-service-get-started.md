...
> **Tip:**
>
> TiDB Cloud provides a Chat2Query API for TiDB Cloud Starter instances. After it is enabled, TiDB Cloud will automatically create a system Data App called **Chat2Query** and a Chat2Data endpoint in Data Service. You can call this endpoint to let AI generate and execute SQL statements by providing instructions.
>
...
Before creating a Data App, make sure that you have created a [TiDB Cloud Starter](/tidb-cloud/select-cluster-tier.md) instance hosted on AWS. If you do not have one, follow the steps in [Create a TiDB Cloud Starter or Essential Instance](/tidb-cloud/create-tidb-cluster-serverless.md) to create one.

> **Note:**
>
> Data Service is available only for [TiDB Cloud Starter](/tidb-cloud/select-cluster-tier.md) instances hosted on AWS. To use Data Service in TiDB Cloud Dedicated clusters, contact [TiDB Cloud Support](/tidb-cloud/tidb-cloud-support.md).
...
3. In the dialog, update the App name if necessary, select TiDB Cloud Starter instances that you want the Data App to access, and then click **Create**.

    The creation process takes a few seconds.

    > **Note:**
    >
    > If there is no TiDB Cloud Starter instance in your current project, you can click **Create New Resource** in the **Link Data Sources** drop-down list to create one first.
...
3. In the **Create Data App** dialog, enter a name, a description, and select TiDB Cloud Starter instances that you want the Data App to access.
...
1. Select a TiDB Cloud Starter instance.

    > **Note:**
    >
    > Only TiDB Cloud Starter instances that are linked to the Data App are displayed in the drop-down list. To manage the linked TiDB Cloud Starter instances, see [Manage linked data sources](/tidb-cloud/data-service-manage-data-app.md#manage-linked-data-sources).

    On the upper part of the SQL editor, select a TiDB Cloud Starter instance on which you want to execute SQL statements from the drop-down list. Then, you can view all databases of this TiDB Cloud Starter instance in the **Schema** tab on the right pane.
...
3. In the **Create API Key** dialog box, do the following:
    ...
    2. Select a role for your API key.
        ...
        The role is used to control whether the API key can read or write data to the TiDB Cloud Starter instances linked to the Data App. You can select the `ReadOnly` or `ReadAndWrite` role:
...
2. In the dialog box, select the TiDB Cloud Starter instance and database that you want to use to call the endpoint, and then copy the code example.
    ...
    > **Note:**
    >
    > - By requesting the regional domain `<region>.data.tidbcloud.com`, you can directly access the endpoint in the region where the TiDB Cloud Starter instance is located.
