...
- **Backup Retention** is the duration for which backups are retained. Expired backups cannot be restored.
    ...
    - For a free MySQL-compatible TiDB Cloud Starter instance, the backup retention is 1 day.
...
- **Backup Time** is the time when the backup starts to be scheduled. Note that the final backup time might fall behind the configured backup time.
    ...
    - For a free MySQL-compatible TiDB Cloud Starter instance, the backup time is a randomly fixed time.
...
5. Update the capacity as needed.
    ...
    - For a MySQL-compatible TiDB Cloud Starter instance, if you need more resources than the [free quota](/tidb-cloud/select-cluster-tier.md#usage-quota), set a monthly spending limit.
...
> **Note:**
>
> TiDB Cloud Starter does not support restoring from Recycle Bin.

To restore a deleted TiDB Cloud Essential instance from the Recycle Bin, take the following steps:
...
2. On the **Recycle Bin** page, click the **Essential** tab to go to the recycle bin of TiDB Cloud Essential instances.

3. Locate the TiDB Cloud Essential instance you want to restore, and then click the **>** button to expand the available backups of the instance.

    > **Note:**
    >
    > If a TiDB Cloud Essential instance **has no backup**, the deleted instance is not displayed in the Recycle Bin.
