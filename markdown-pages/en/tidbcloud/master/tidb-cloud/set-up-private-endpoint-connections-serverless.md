...
# Connect to TiDB Cloud Starter via AWS PrivateLink

This document describes how to connect to your TiDB Cloud Starter cluster via AWS PrivateLink.
...
- Currently, TiDB Cloud supports AWS PrivateLink connections to TiDB Cloud Starter only when the endpoint service is hosted in AWS. If the service is hosted in other cloud providers, the AWS PrivateLink connection is not applicable.
...
1. On the [**Clusters**](https://tidbcloud.com/project/clusters) page, click the name of your target TiDB Cloud Starter cluster to go to its overview page.
...
4. Take a note of **Service Name**, **Availability Zone ID**, and **Region ID**.
    ...
    > **Note:**
    >
    > You only need to create one private endpoint per AWS region, which can be shared by all TiDB Cloud Starter clusters located in the same region.
