...
> **Tip:**
>
> `{version}` in the link indicates the version number of TiDB and `{arch}` indicates the architecture of the system, which can be `amd64` or `arm64`. For example, the download link for `vv8.5.0` in the `amd64` architecture is `https://download.pingcap.com/tidb-community-toolkit-vv8.5.0-linux-amd64.tar.gz`.
...
3. Deploy a TiDB cluster:
    ...
    ```shell
    tiup cluster deploy tidb-test vv8.5.0 ./topology.yaml --user root [-p] [-i /home/root/.ssh/gcp_rsa]
    ```

In the `tiup cluster deploy` command above:

- `tidb-test` is the name of the TiDB cluster to be deployed.
- `vv8.5.0` is the version of the TiDB cluster to be deployed. You can see the latest supported versions by running `tiup list tidb`.
