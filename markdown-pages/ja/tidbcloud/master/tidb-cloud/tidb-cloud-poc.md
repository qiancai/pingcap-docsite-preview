...
Alternatively, you can [create a TiDB Cloud Starter](/tidb-cloud/tidb-cloud-quickstart.md#step-1-create-a-tidb-cluster) to get familiar with TiDB Cloud for a quick evaluation. Note that the TiDB Cloud Starter has some [special terms and conditions](/tidb-cloud/serverless-limitations.md).
...
Because the amount of PoC credits is limited, to maximize the value of credits, it is recommended that you create a [TiDB Cloud Starter cluster](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) for compatibility tests and preliminary analysis on TiDB Cloud.
